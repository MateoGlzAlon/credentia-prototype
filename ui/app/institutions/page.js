  ...
  async function getInstitutionsContract() {
    try {
      ...
      for (const addr of addrs) {
        ...
        const image = await institution.institutionLogo(); // <--- NUEVO
        let diplomasEmitidos = "No disponible";

        // Si añadiste en el contrato:
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <h3 className="text-xl font-semibold text-foreground mb-2">No hay instituciones registradas</h3>
            <p className="text-muted-foreground mb-6">Sé la primera universidad en registrarse en la plataforma</p>
            <Link href="/create"
              ...
              <div key={inst.address}
                ...
                <div className="space-y-4">
                  {inst.image && inst.image.startsWith("http") ? (
                    <img src={inst.image} alt={inst.name} className="w-full h-32 object-contain rounded" />
                  ) : (
                  ...
                  <Link href={`/institutions/${inst.address}`}
                    className="block w-full mt-4 px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 text-center">
                    Ver Detalles
                  </Link>
                </div>
              </div>
